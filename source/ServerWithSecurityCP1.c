    ...
    while (1)
    {
        ...
        case MSG_FILE_DATA:
        {
            ...
            */
            ...
            if(!private_key){
                free(enc_file_data);
                printf("error: no damn key found or some shi");
                break;
            }

            size_t max_dec_len = (enc_file_len/128)*62;
            unsigned char *file_data = malloc(max_dec_len);
            size_t file_len = 0;
            size_t read_data = 0;
            
            while(read_data<enc_file_len){
                size_t out_len = 0;
                unsigned char *dec_part = rsa_decrypt_block(private_key, enc_file_data+read_data, 128, &out_len, 1);
                memcpy(file_data+file_len, dec_part, out_len);
                file_len += out_len;
                read_data += 128;
                free(dec_part);
            }
